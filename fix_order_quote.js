...
code = code.replace(/const deliveryFeeBaseAmount = 15\.0;/g, 'const deliveryFeeBaseAmount = pricingConfig.minimumCustomerDeliveryFee;');
code = code.replace(/const deliveryFeePerExtraKm = 5\.0;/g, 'const deliveryFeePerExtraKm = pricingConfig.customerDeliveryPerKm;');
...
code = code.replace(/const totalCustomerTaxes = 0\.0;/g, 'const totalCustomerTaxes = Math.round(foodSubtotal * ((pricingConfig.foodGstRate || 0) / 100) * 100) / 100;');
code = code.replace(/taxes: totalCustomerTaxes, \/\/ Hardcoded to 0 for now/g, 'taxes: totalCustomerTaxes,');

// Get config at the top of calculateQuote
code = code.replace(/async calculateQuote\(req: OrderQuoteRequest\): Promise<OrderQuoteResponse> \{/, "async calculateQuote(req: OrderQuoteRequest): Promise<OrderQuoteResponse> {\n    const pricingConfig = await this.pricingService.getActivePricingConfig();");
