...
const regex = /if \(isNaN\(lat\) \|\| isNaN\(lng\)\) \{[\s\S]*?return \{[\s\S]*?success: false,[\s\S]*?debugData: data,[\s\S]*?debugFirstResult: firstResult,[\s\S]*?reason: 'Mappls returned invalid coordinates'[\s\S]*?\} as any;\s*\}/;
...
code = code.replace(/longitude: lng,/g, 'longitude: lngResolved,');
code = code.replace(/latitude: lat,/g, 'latitude: latResolved,');

fs.writeFileSync('apps/backend/src/modules/geolocation/geolocation.service.ts', code);
console.log('Modified with eLoc fallback');
