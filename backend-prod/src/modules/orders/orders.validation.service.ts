import { getRestaurantAvailability } from '../restaurants/restaurant-availability.util';
...
export class OrdersValidationService {
  ...
  async validateRestaurantOpen(restaurantId: string): Promise<void> {
    ...
    const { isWithinHours } = getRestaurantAvailability(restaurant.timings, restaurant.isOpen);
    if (!isWithinHours) {
      throw new BadRequestException('Restaurant is currently outside business hours');
    }
