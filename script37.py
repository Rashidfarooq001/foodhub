...
# 2. Fetch Block
old_fetch = '''        const fetchCurrent = fetch(`${API_BASE}/delivery/current?_t=${Date.now()}`, { headers, cache: 'no-store' })
          .then(async r => {
            if (r.ok) {
...
new_fetch = '''        const fetchActiveJobs = fetch(`${API_BASE}/delivery/active-jobs?_t=${Date.now()}`, { headers, cache: 'no-store' })
          .then(async r => {
            if (r.ok) {
              const text = await r.text();
              try {
