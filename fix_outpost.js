...
const regex = /const elocResponse = await fetch\(elocUrl, \{ headers: elocHeaders, signal: elocController\.signal \}\);[\s\S]*?if \(elocResponse\.ok\) \{[\s\S]*?const elocData = await elocResponse\.json\(\);/;
