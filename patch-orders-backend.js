...
const regex = /let routeCoordinates: \[number, number\]\[\] = \[\];[\s\S]*?etaMins = routeData\.etaMinutes;\s+\} catch \(err: any\) \{[\s\S]*?\}\s+\}/;
...
content = content.replace(/(const driverLat = order\.tracking\?\.currentLat \? Number\(order\.tracking\.currentLat\) : null;)/, 'let driverLat = order.tracking?.currentLat ? Number(order.tracking.currentLat) : null;');
content = content.replace(/(const driverLng = order\.tracking\?\.currentLng \? Number\(order\.tracking\.currentLng\) : null;)/, 'let driverLng = order.tracking?.currentLng ? Number(order.tracking.currentLng) : null;');
