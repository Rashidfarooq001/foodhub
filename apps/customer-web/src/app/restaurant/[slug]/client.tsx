...
export default function RestaurantDetailPage() {
        ...
        if (res.ok) {
          ...
          if (data?.id) {
            try {
              ...
              if (revRes.ok) {
                const rv = await revRes.json();
                setReviewsList(rv.reviews || []);
              }
            } catch {
              /* ignore */
            }
  ...
  const categoriesList = useMemo(() => {
    ...
    restaurant.foodItems.forEach((f) => {
      ...
      <div className="mx-auto max-w-md px-4 py-10 text-center">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm space-y-4">
          ...
          <p className="text-xs text-gray-500">
            This kitchen is unavailable or doesn&apos;t exist.
          </p>
          <Link
            href="/"
            className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-rose-700 transition"
          >
    ...
    <div className="bg-gray-50/50 pb-8">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-2.5 shadow-sm">
        <div className="mx-auto max-w-4xl flex items-center gap-3">
          <Link
            href="/"
            className="flex h-8 w-8 items-center justify-center rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 transition"
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-gray-500 truncate">
              {restaurant.cuisines?.join(' • ') || 'Multi-Cuisine Kitchen'}
            </p>
          </div>
        </div>
      </div>
      ...
      <main className="mx-auto max-w-4xl px-3 sm:px-4 py-3 space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 sm:p-3 text-[11px] text-gray-600 bg-gray-50/70 border-t border-gray-100">
            ...
            <div className="flex items-center gap-3 shrink-0 font-semibold">
              <div className="flex items-center gap-1">
                ...
                <span>
                  {restaurant.deliveryTimeMins ? restaurant.deliveryTimeMins + ' mins' : '30 mins'}
                </span>
              </div>
        ...
        <section className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none -mx-0.5 px-0.5">
          ...
          >
            <span
              className={`h-2 w-2 rounded-full ${isVegOnly ? 'bg-emerald-600 ring-1 ring-emerald-600' : 'bg-gray-400'}`}
            />
            Veg Only
          </button>
                ...
                <div key={rev.id} className="pt-2 text-[11px] space-y-0.5">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900">
                      {rev.customer?.user?.profile?.firstName || 'Verified Customer'}
                    </span>
                    <div className="flex text-amber-400">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          className={`h-3 w-3 ${s <= rev.rating ? 'fill-amber-400' : 'text-gray-200'}`}
                        />
                      ))}
                    </div>
                  </div>
      ...
      {/* MENU SHORTCUT FAB */}
      <div className={`fixed right-4 sm:right-6 z-[100] transition-all duration-300 ${cartItemCount > 0 ? "bottom-[124px] sm:bottom-[76px]" : "bottom-[72px] sm:bottom-8"}`}>
        <button
      ...
      {cartItemCount > 0 && (
        <div className="fixed bottom-16 sm:bottom-4 left-3 right-3 z-40 max-w-lg mx-auto">
          <Link
            href="/cart"
            className="flex items-center justify-between rounded-2xl bg-rose-600 px-4 py-3 text-white shadow-xl shadow-rose-600/30 hover:bg-rose-700 transition active:scale-[0.99]"
          >
            <div className="flex items-center gap-2.5">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/20 font-black text-xs">
                {cartItemCount}
              </div>
              <div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                {!isAvail ? (
                                  ...
                                  <>
                                    {((food.variants?.length || 0) > 0 || (food.addonGroups?.length || 0) > 0) && (
          ...
          <div className="w-full max-w-md rounded-t-3xl sm:rounded-2xl bg-white p-5 shadow-2xl space-y-4 max-h-[88vh] overflow-y-auto">
                  ...
                  {selectedFood.variants.map((v) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <div
                            className={`h-4 w-4 rounded-full border flex items-center justify-center ${sel ? 'border-rose-600 bg-rose-600' : 'border-gray-300'}`}
                          >
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100">
              ...
              <div className="flex items-center gap-2 rounded-xl bg-gray-100 p-1">
                <button
                  type="button"
                  onClick={() => setCustomizationQty((q) => Math.max(1, q - 1))}
                  disabled={customizationQty <= 1}
                  className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-gray-700 shadow-sm disabled:opacity-40 transition"
                >
                  <Minus className="h-3.5 w-3.5" />
                </button>
                <span className="w-5 text-center text-xs font-black text-gray-900">
                  {customizationQty}
                </span>
                <button
                  type="button"
                  onClick={() => setCustomizationQty((q) => Math.min(20, q + 1))}
                  className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-gray-700 shadow-sm transition"
                >
            ...
            <button
              ...
              disabled={
                !!(selectedFood.variants && selectedFood.variants.length > 0 && !selectedVariant)
              }
            ...
            >
              ...
              <span className="font-black">
                Rs.{' '}
                {((selectedVariant ? selectedVariant.price : selectedFood.price) +
