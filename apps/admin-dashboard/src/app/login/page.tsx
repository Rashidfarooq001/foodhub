...
import { ShieldCheck, Lock, Mail, ArrowRight, Phone, CheckCircle2, RotateCcw, Edit2 } from 'lucide-react';
  ...
  const handleWidgetSuccess = async (accessToken: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/auth/verify-otp`, {
        ...
        body: JSON.stringify({ accessToken, targetRole: 'ADMIN' }),
      });
      ...
      setError(err.message || 'OTP verification failed for admin command center');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    if (typeof window !== 'undefined' && typeof (window as any).initSendOTP === 'function') {
      try {
        ...
        if (typeof (window as any).sendOtp === 'function') {
          (window as any).sendOtp(identifier, () => {}, (err: any) => console.error('[MSG91 Admin] sendOtp error:', err));
        }
        setStep('OTP');
        setCooldown(30);
        setIsLoading(false);
        ...
        console.warn('[MSG91 Admin] initSendOTP exception:', widgetErr?.message || widgetErr);
      }
    }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (typeof window !== 'undefined' && typeof (window as any).verifyOtp === 'function') {
      try {
        ...
        console.warn('[MSG91 Admin] verifyOtp exception:', verifyErr);
      }
    }
  };
      ...
      setAuth(
        {
          ...
          email: data.user.email,
          role: data.user.role,
          name: data.user.profile?.firstName || 'Super Admin',
        },
        data.tokens.accessToken,
      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl bg-white p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-2 rounded-2xl bg-gray-100 p-1 text-xs font-bold">
          <button
            ...
            className={`rounded-xl py-2.5 transition ${loginMode === 'OTP' ? 'bg-white text-purple-600 shadow' : 'text-gray-500 hover:text-gray-900'}`}
          >
            MSG91 OTP Login
          </button>
          <button
            ...
            className={`rounded-xl py-2.5 transition ${loginMode === 'PASSWORD' ? 'bg-white text-purple-600 shadow' : 'text-gray-500 hover:text-gray-900'}`}
          >
            Password Login
          </button>
        </div>
        ...
        {loginMode === 'OTP' ? (
          step === 'PHONE' ? (
            <form onSubmit={handleSendOtp} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Registered Admin Phone</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Enter registered 10-digit admin mobile"
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-5">
              <div className="flex justify-center gap-3">
                {otp.map((digit, idx) => (
                  <input
                    ...
                    className="h-12 w-12 rounded-2xl border-2 border-gray-200 text-center text-lg font-black text-gray-900 focus:border-purple-600 focus:outline-none"
                  />
                ))}
              </div>
              ...
              >
                ...
                <span>{isLoading ? 'Verifying...' : 'Verify & Access Command Center'}</span>
              </button>
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                <button
                  ...
                  className="flex items-center gap-1 font-bold text-gray-500 hover:text-purple-600"
                >
          ...
          <form onSubmit={handlePasswordLogin} className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Admin Phone</label>
              <div className="relative">
