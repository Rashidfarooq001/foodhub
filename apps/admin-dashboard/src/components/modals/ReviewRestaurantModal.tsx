...
export default function ReviewRestaurantModal({
  restaurantId,
  onClose,
  onApprove,
  onReject,
}: ReviewRestaurantModalProps) {
  ...
  const owner =
    data.staff?.find((s: any) => s.designation?.toUpperCase() === 'OWNER')?.user || null;
        ...
        <div className="flex-none bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 sm:rounded-t-2xl">
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium mt-1">
              Application ID: <span className="font-mono text-xs">{data.id}</span>
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="grid grid-cols-3 gap-2 border-b pb-2">
                  ...
                  <span className="col-span-2 text-gray-900 leading-relaxed">
                    {data.addressLine}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-2 border-b pb-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <span className="col-span-2 text-gray-900 font-mono">
              ...
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-3 gap-2 border-b pb-2">
                  ...
                  <span className="col-span-2 font-mono font-bold text-gray-900">
                    {data.licenseFssai || 'Not provided'}
                  </span>
                </div>
                {data.fssaiUrl && (
                  <div className="grid grid-cols-3 gap-2 border-b pb-2">
                ...
                {data.panUrl && (
                  <div className="grid grid-cols-3 gap-2 border-b pb-2">
                    <span className="text-gray-500 font-medium">PAN Doc</span>
                    <span className="col-span-2">
                      <a
                        href={data.panUrl}
                        target="_blank"
                ...
                {data.documents &&
                  data.documents.map((doc: any) => {
                    // Prevent duplicate rendering if the URL is exactly the same as the top-level fields
                    if (doc.documentUrl === data.fssaiUrl && doc.documentType === 'FSSAI')
                      return null;
                    if (doc.documentUrl === data.panUrl && doc.documentType === 'PAN') return null;
