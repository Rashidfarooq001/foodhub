...
async function main() {
  ...
  if (adminUsers.length === 0) {
    throw new Error(
      'CRITICAL SAFETY BLOCK: No Admin user found! Aborting cleanup to prevent lockout.',
    );
  }
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        `DELETE FROM "wallet_transactions" WHERE "wallet_id" IN (SELECT id FROM "wallets" WHERE "user_id" NOT IN (${adminIds}));`,
      );
      await tx.$executeRawUnsafe(`DELETE FROM "wallets" WHERE "user_id" NOT IN (${adminIds});`);

      // M. Audit logs (Except Admin)
      await tx.$executeRawUnsafe(
        `DELETE FROM "audit_logs" WHERE "user_id" IS NULL OR "user_id" NOT IN (${adminIds});`,
      );
      ...
      await tx.$executeRawUnsafe(
        `DELETE FROM "users" WHERE "id" NOT IN (${adminIds}) AND "role"::text NOT IN ('ADMIN', 'SUPER_ADMIN');`,
      );
    },
    {
      timeout: 60000,
