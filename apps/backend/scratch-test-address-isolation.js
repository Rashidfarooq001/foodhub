...
function createAddressStore() {
  ...
  return {
    ...
    addAddress: (newAddr) => {
      const isCurrentLoc = newAddr.id === 'current-location' || newAddr.label === 'Current Location';
...
async function runTests() {
  ...
  store.addAddress({ id: 'addr-gps-1', label: 'Current Location', latitude: 34.3868, longitude: 74.5221, addressLine1: 'Bandipora-Sopore Road' });
  store.addAddress({ id: 'addr-gps-2', label: 'Current Location', latitude: 34.3870, longitude: 74.5225, addressLine1: 'Bandipora-Sopore Road' });
  store.addAddress({ id: 'addr-gps-3', label: 'Current Location', latitude: 34.3875, longitude: 74.5230, addressLine1: 'Bandipora-Sopore Road' });

  const currentAddrs = store.getAddresses();
  console.log(`Address count in store after 3 GPS updates: ${currentAddrs.length}`);
  console.log(`Current Location entry ID: "${currentAddrs[0]?.id}" | Coordinates: (${currentAddrs[0]?.latitude}, ${currentAddrs[0]?.longitude})`);
  ...
  console.log('✓ Current Location duplication prevention verified (Exactly 1 entry with stable ID "current-location").');

  console.log('\n=== TEST 2: CUSTOMER ADDRESS SESSION ISOLATION ===');
  // Add Customer A Home address
  store.addAddress({ id: 'custA-home', label: 'Home', addressLine1: 'Sopore Main Town', city: 'Sopore', state: 'J&K', latitude: 34.3868, longitude: 74.5221 });
  ...
  console.log(`DB Stored City: "${dbAddr.city}" | Coordinates: (${dbAddr.latitude}, ${dbAddr.longitude})`);

  if (dbAddr.city === 'Bengaluru' || dbAddr.latitude === 12.9780) {
