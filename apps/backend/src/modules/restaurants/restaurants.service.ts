import { getRestaurantAvailability } from './restaurant-availability.util';
import {
  Injectable,
  NotFoundException,
  BadRequestException,
...
export class RestaurantsService {
  ...
  // --- NEW OPERATING STATUS LOGIC ---
  async getRestaurantOperatingStatus(restaurantId: string): Promise<{
  ...
  async updateOnlineStatus(restaurantId: string, isOpen: boolean, user: any) {
    await this.verifyRestaurantAccess(restaurantId, user);
    
    const status = await this.getRestaurantOperatingStatus(restaurantId);
    
    if (isOpen && !status.isCurrentlyOpen && !status.isManuallyOffline) {
       throw new BadRequestException('Cannot go online outside configured business hours.');
    }
    ...
    if (this.gateway) {
      this.gateway.server.emit(ORDER_EVENTS.RESTAURANT_STATUS_CHANGED, {
        restaurantId,
