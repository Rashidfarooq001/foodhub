...
export default function RestaurantForgotPasswordPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    if (cleanDigits.length < 10) {
      setError('Please enter a valid 10-digit registered restaurant phone number.');
      return;
    }
    ...
    try {
      // 1. Initiate password reset for phone
      const res = await fetch(`${API_BASE}/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          phone: formattedPhone,
          targetRole: 'HOTEL', // Restricts recovery strictly to Restaurant Roles
        }),
      });
      ...
      if (!res.ok) {
        setError(data.message || 'No authorized restaurant account found for this phone number.');
        return;
      }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-gray-950 px-4 py-12">
      <div className="w-full max-w-md space-y-8 rounded-3xl bg-white p-8 shadow-2xl border border-gray-100">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black text-gray-900">Restaurant Password Recovery</h1>
          <p className="text-xs text-gray-500">
            Reset credentials for RESTAURANT_OWNER, MANAGER &amp; STAFF accounts
          </p>
        </div>
        ...
        {step === 'PHONE' && (
          <form onSubmit={handleRequestOtp} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-gray-200 py-3.5 pl-10 pr-4 text-xs font-bold text-gray-900 focus:border-orange-600 focus:outline-none"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-600 py-4 text-xs font-black uppercase text-white shadow-lg shadow-orange-600/25 hover:bg-orange-700 disabled:opacity-50 transition"
            >
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border-2 border-orange-200 py-3 text-center text-lg font-black text-gray-900 focus:border-orange-600 focus:outline-none tracking-widest"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-600 py-4 text-xs font-black uppercase text-white shadow-lg hover:bg-orange-700 disabled:opacity-50 transition"
            >
        ...
        {step === 'SUCCESS' && (
          <div className="text-center space-y-6">
            ...
            <button
              ...
              className="w-full rounded-2xl bg-orange-600 py-4 text-xs font-black uppercase text-white shadow-lg hover:bg-orange-700 transition"
            >
