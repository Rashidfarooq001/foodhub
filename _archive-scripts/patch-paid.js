...
replaceInFile('apps/backend/src/modules/payments/payments.service.ts', (c) => {
  let nc = c.replace(
    /select: { restaurantId: true, paidAmount: true,/g,
    'select: { restaurantId: true, netPayable: true, status: true,',
  );
  nc = nc.replace(
    /const totalRestaurantSettled = settlements\.reduce\(\(sum, s\) => sum \+ Number\(s\.paidAmount \|\| 0\), 0\);/g,
...
replaceInFile('apps/backend/src/modules/analytics/analytics.service.ts', (c) => {
  return c.replace(
    /Number\(s\.paidAmount \|\| s\.netPayable \|\| 0\)/g,
    'Number(s.netPayable || 0)',
  );
});
