...
content = content.replace(
  /h-\[350px\] md:h-\[400px\] lg:h-\[500px\]/g,
  'h-[45vh] md:h-[50vh] lg:h-[600px] relative',
);
...
`;
content = content.replace(
  /<DynamicLiveTrackingMap[^>]+>[\s\S]*?<\/DynamicLiveTrackingMap>/,
  trackButtonHtml.trim(),
);

// Fix ETA Display
const etaRegex =
  /<div className="flex items-center gap-3 rounded-2xl bg-orange-50 px-5 py-3 text-orange-800 ring-1 ring-orange-200">[\s\S]*?<\/div>\s*<\/div>/;
...
const driverCardRegex =
  /<span\s*className=\{\`rounded-full px-2.5 py-0.5 text-\[10px\] font-bold uppercase \$\{\s*isDriverAssigned \? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'\s*\}\`\}\s*>[\s\S]*?<\/span>/;
