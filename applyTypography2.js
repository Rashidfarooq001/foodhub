...
content = content.replace(
  /className="text-\[10px\] font-black text-orange-600 "/g,
...
content = content.replace(
  /className="font-black text-gray-900 shrink-0"/g,
  'className="font-semibold text-gray-900 shrink-0"'
);
content = content.replace(
  /className="font-black text-gray-900"/g,
...
// We will replace exactly the specific container div
const startIdx = content.indexOf('{/* 9. STICKY BOTTOM CTA */}');
const endIdx = content.indexOf('{/* MANUAL ADDRESS MODAL */}');

if (startIdx !== -1 && endIdx !== -1) {
  const oldStickyCTA = content.substring(startIdx, endIdx);
  const newStickyCTA = `{/* 9. STICKY BOTTOM CTA */}
        ...
        `;
  content = content.substring(0, startIdx) + newStickyCTA + content.substring(endIdx);
}
...
console.log('Typography hierarchy applied successfully');
