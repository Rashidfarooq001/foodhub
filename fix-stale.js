...
content = content.replace(/setDriverLoc\(\{\s*lat:\s*data.lat,\s*lng:\s*data.lng\s*\}\);/g, `setDriverLoc({ lat: data.lat, lng: data.lng });\n          setLastUpdate(new Date());`);
content = content.replace(/setDriverLoc\(\{\s*lat:\s*trackData.driverLat,\s*lng:\s*trackData.driverLng\s*\}\);/g, `setDriverLoc({ lat: trackData.driverLat, lng: trackData.driverLng });\n            if (trackData.updatedAt) setLastUpdate(new Date(trackData.updatedAt));\n            else setLastUpdate(new Date());`);

// 2. Change the Live Socket Connected text to reflect stale status
const socketTextRegex = /<span className=\{\`h-2 w-2 rounded-full \$\{\s*isSocketConnected \? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'\s*\}\`\} \/>\s*\{isSocketConnected \? 'Live Socket Connected' : 'Auto Syncing \(5s\)'\}/;
