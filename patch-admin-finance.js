...
const file = path.join('apps', 'admin-dashboard', 'src', 'app', 'settlements', 'page.tsx');
...
export default function AdminFinancePage() {
  ...
  const filteredRiders = riders.filter((r: any) => {
    const name = \`\${r.driver.user?.profile?.firstName || ''} \${r.driver.user?.profile?.lastName || ''}\`.trim();
    ...
    <div className="p-8 max-w-7xl mx-auto text-slate-900 bg-slate-50 min-h-screen">
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage real-time payouts and ledger balances from one authoritative source.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-4 bg-amber-50 text-amber-600 rounded-xl"><CreditCard size={24} /></div>
          <div>
            <p className="text-sm text-slate-500 font-medium">Pending Restaurants</p>
            <p className="text-2xl font-bold text-amber-600">?{(stats.pendingRestaurantSettlements || 0).toFixed(2)}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-4 bg-blue-50 text-blue-600 rounded-xl"><Bike size={24} /></div>
          <div>
            <p className="text-sm text-slate-500 font-medium">Pending Riders</p>
            <p className="text-2xl font-bold text-blue-600">?{(stats.pendingRiderSettlements || 0).toFixed(2)}</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-white">
          ...
          <div className="flex gap-4">
            ...
            <div className="relative w-64">
              ...
              <input 
                type="text"
                placeholder={activeTab === 'restaurant' ? "Search restaurant..." : "Search rider..."}
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-sm">
                <th className="p-4 font-medium border-b border-slate-100">{activeTab === 'restaurant' ? 'Restaurant' : 'Rider'}</th>
                <th className="p-4 font-medium border-b border-slate-100">{activeTab === 'restaurant' ? 'Orders' : 'Deliveries'}</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="p-8 text-center text-slate-500">No riders found.</td>
                </tr>
              ) : activeTab === 'restaurant' ? (
                filteredRestaurants.map((r: any) => (
                  <tr key={r.restaurant.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={\`px-3 py-1 rounded-full text-xs font-medium \${r.status === 'PENDING' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}\`}>
                        {r.status}
                      </span>
                    </td>
                    <td className="p-4">
                      <Link href={\`/finance/restaurant/\${r.restaurant.id}\`} className="text-purple-600 font-medium hover:text-purple-800 text-sm">
                        View Details ?
                      </Link>
                    </td>
                  </tr>
                ...
                filteredRiders.map((r: any) => (
                  <tr key={r.driver.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="p-4 font-bold">{\`\${r.driver.user?.profile?.firstName || ''} \${r.driver.user?.profile?.lastName || ''}\`.trim()}</td>
                    <td className="p-4 text-slate-600">{r.completedDeliveries}</td>
                    <td className="p-4 font-medium">?{(r.totalEarnings || 0).toFixed(2)}</td>
                    <td className="p-4 font-bold text-slate-900">?{(r.totalEarnings || 0).toFixed(2)}</td>
                    <td className="p-4 text-green-600 font-medium">?{(r.paidAmount || 0).toFixed(2)}</td>
                    <td className="p-4 text-red-600 font-bold">?{(r.pendingAmount || 0).toFixed(2)}</td>
                    <td className="p-4">
                      <span className={\`px-3 py-1 rounded-full text-xs font-medium \${r.settlementStatus === 'PENDING' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}\`}>
                        {r.settlementStatus}
                      </span>
                    </td>
                    <td className="p-4">
                      <Link href={\`/finance/rider/\${r.driver.id}\`} className="text-purple-600 font-medium hover:text-purple-800 text-sm">
                        View Details ?
                      </Link>
                    </td>
                  </tr>
...
console.log("Patched admin-dashboard finance page.");
