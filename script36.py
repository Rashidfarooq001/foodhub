...
# 2. Fetch block
old_fetch = r"const fetchCurrent = fetch.*?\.catch\(e => \{\s*console\.error\(\"Network error fetching current delivery:\", e\);\s*setCurrentDelivery\(null\);\s*\}\);"
new_fetch = r"""const fetchActiveJobs = fetch(${API_BASE}/delivery/active-jobs?_t=, { headers, cache: 'no-store' })
          .then(async r => {
            if (r.ok) {
...
# 4. Banner logic
old_banner = r"\{/\* ACTIVE DELIVERY IN PROGRESS BANNER.*?\}\)"
new_banner = r"""{/* ACTIVE DELIVERIES */}
