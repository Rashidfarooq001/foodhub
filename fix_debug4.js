...
const regex = /let finalLat = lat;[\s\S]*?let finalLng = lng;[\s\S]*?\/\/ If Geocode API only returns/;
...
const regex4 = /if \(isNaN\(finalLat\) \|\| isNaN\(finalLng\)\) \{[\s\S]*?return \{[\s\S]*?success: false,[\s\S]*?debugData: \{ elocStatus: elocResponse\?\.status, elocUrl: elocUrl, firstResult: firstResult \},[\s\S]*?reason: 'Mappls returned invalid coordinates\. ' \+ \(firstResult\.eLoc \? 'Failed to resolve eLoc\.' : ''\)[\s\S]*?\} as any;\s*\}/;
